...
export const introduces = {
  ...
  table: ['图表界面首先是使用echarts生成的图(屏幕自适应),让天气可以更加直观的显示', '天气数据使用的是和风天气提供的api，使用了查询未来天气和地理位置的接口，且可以切换城市显示，默认显示城市为当前登陆用户的城市', '表使用的是element的table组件,表头使用自定义且加上了图标，且使用了xlxs和file-saver实现了表的导出'],
